...
$(document).ready(() => {
    ...
    LoadAllJsonObjects(jsonList, db).then(() => {
        ...
        for (let i=0; i<db.chars2.length; ++i) {
            let curr_char = db.chars2[i]
            let char_id = curr_char.id
            let curr_info = db.handbookInfo.handbookDict[char_id]

            op_dict[char_id] = curr_char
            op_dict[char_id].info = curr_info
            //delete op_dict[char_id].id
        }
